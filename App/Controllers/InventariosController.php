...
if(!empty($_GET['action'])){
    InventariosController::main($_GET['action']);
}
...
class InventariosController
{
    ...
    static public function create()
    {
        try {
            ...
            $arrayInventario['Precio'] = $_POST['Precio'];
    ...
    static public function edit()
    {
        try {
            ...
            header("Location: ../../views/modules/Inventario/show.php?Codigo=" . $Inventario->getCodigo() . "&respuesta=correcto");
        } catch (\Exception $e) {
    ...
    {
        try {
            $ObjUsuario = Inventario::searchForDocumento($_GET['Id']);
    ...
    static public function inactivate()
    {
        try {
            ...
            //var_dump($e);
            header("Location: ../../views/modules/Inventario/index.php?respuesta=error");
        }
    }

    public static function searchForDocumento($Codigo){
        try {
            return Inventario::searchForDocumento($Codigo);
        } catch (\Exception $e) {
            var_dump($e);
    ...
    static public function selectUsuario ($isMultiple=false,
        ...
        if(count($arrUsuarios) > 0){
            foreach ($arrUsuarios as $usuario)
                if (!InventariosController::InventarioIsInArray($usuario->getCodigo(),$arrExcluir))
